...
const pdfGenerator = async (patientData, index) => {
  ...
  doc.text(`${index}/GENOSE/${moment().format('MMMM').toUpperCase()}/${moment().format('YYYY')}`, 105, 110+add)
  ...
  doc.rect(width/4+45, 185+add, width - (width/4 + (40*2)), 15, 'F')
  doc.cell(40, 185+add, width/4+5, 15, 'Prediction')

  doc.setFont('times', 'bold')
  doc.cell(width/4+45, 185+add, width - (width/4 + (40*2)), 15, ' ')
  doc.text(width/4+48, 195+add, ': ' + result) // Hasil
