...
const pdfGenerator = async (patientData, index) => {
  const { name, gender, dob, pob, address, checkupDate, time, result } = patientData
  const now = moment()
  const date = moment(dob, 'MM-DD-YY')
  ...
  doc.text(":", 100, 110)
  doc.text("920/ANTIGEN/JUNE/2021", 105, 110)
  
  doc.text("Date", 45, 120)
  doc.text(":", 100, 120)
  doc.text(moment(new Date).format('DD MMMM YYYYY'), 105, 120)
  ...
  doc.cell(width/4+45, 170, 80, 15, moment(dob, 'MM-DD-YY').format('DD MMMM YYYY')) // Birth Date
  doc.cell(width/4+45+80, 170, 75, 15, `${age} Tahun`) // Age
  doc.cell(width/4+39.6+(80*2), 170, 80, 15, gender) // Sex

  doc.setFillColor(238,238,238)
  doc.rect(width/4+45, 185, width - (width/4 + (40*2)), 25, 'F')
  doc.cell(40, 185, width/4+5, 25, 'SARS-CoV-Antigen (Covid19)\nRapid Test Result')

  doc.setFont('times', 'bold')
  doc.cell(width/4+45, 185, width - (width/4 + (40*2)), 25, ' ')
  doc.text(width/4+48, 200, result) // Hasil
